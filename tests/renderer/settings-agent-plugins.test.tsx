...
describe('SettingsContent agent capability plugins', () => {
  ...
  it('renders built-in plugin summaries without default detail noise', async () => {
    ...
    const heading = await screen.findByText('Agent Capability Plugins')
    const section = heading.closest('section')
    expect(section).not.toBeNull()
    const pluginSection = within(section!)
    expect(pluginSection.getByText('Claude Code')).toBeInTheDocument()
    expect(pluginSection.getByText('Codex')).toBeInTheDocument()
    expect(pluginSection.getAllByText('Built-in')).toHaveLength(2)
    expect(pluginSection.getAllByText('Enabled')).toHaveLength(2)
    expect(pluginSection.getByText('Detected')).toBeInTheDocument()
    expect(pluginSection.getByText('No local data')).toBeInTheDocument()
    expect(pluginSection.getAllByText('v0.1.0')).toHaveLength(2)
    expect(pluginSection.getByText('Target: Claude Code')).toBeInTheDocument()
    expect(pluginSection.getByText('2 capabilities')).toBeInTheDocument()
    expect(pluginSection.queryByText('Permissions')).not.toBeInTheDocument()
    expect(pluginSection.queryByText('~/.claude/settings.json')).not.toBeInTheDocument()
  })
  ...
  it('renders manifest summaries without default detail noise', async () => {
    ...
    const firstManifest = await screen.findByText('Claude Helper')
    const section = firstManifest.closest('section')
    expect(section).not.toBeNull()
    const pluginSection = within(section!)
    expect(pluginSection.getByText('Ready Helper')).toBeInTheDocument()
    expect(pluginSection.getByText('Blocked Helper')).toBeInTheDocument()
    expect(pluginSection.getByText('Future Helper')).toBeInTheDocument()
    expect(pluginSection.getByText('Broken Helper')).toBeInTheDocument()
    expect(pluginSection.getAllByText('Manifest')).toHaveLength(5)
    expect(pluginSection.getAllByText('Read-only')).toHaveLength(5)
    expect(pluginSection.getByText('Metadata')).toBeInTheDocument()
    expect(pluginSection.getByText('Ready')).toBeInTheDocument()
    expect(pluginSection.getByText('Blocked')).toBeInTheDocument()
    expect(pluginSection.getByText('Incompatible')).toBeInTheDocument()
    expect(pluginSection.getByText('Invalid')).toBeInTheDocument()
    expect(pluginSection.getAllByText('Target: Claude Code').length).toBeGreaterThan(0)
    expect(pluginSection.getAllByText('Range: >=2.0.0').length).toBeGreaterThan(0)
    expect(pluginSection.queryByText('Activation readiness')).not.toBeInTheDocument()
    expect(pluginSection.queryByText('Validation errors')).not.toBeInTheDocument()
    expect(pluginSection.queryByText('./adapter.js')).not.toBeInTheDocument()
    expect(pluginSection.queryByText('~/.codex/config.toml')).not.toBeInTheDocument()
    expect(pluginSection.queryByText('Store reviewed hook registration changes.')).not.toBeInTheDocument()
    expect(pluginSection.queryByText('permissions.0.kind')).not.toBeInTheDocument()
  })
