...
- `pnpm harness:stats` 汇总项目 debt pool。阈值 (2026-07-04 按实际净增速校准, 原 40/60 从未触达): `<20 ok`, `>=20 notice`, `>=30 recommend-maintenance`, `>=45 requires-override`; 超过 45 继续做非维护任务时, INDEX 必须写 `debt.override_reason`。
- `pnpm harness:stats` 输出 `maintenance=<subtype>:<score>` 的触发条件 (2026-07-04 起**域级触发**): total 达维护阈值, **或任一域自身 `>=30`** — total 是跨域净和, 冷域盈余不得掩蔽热域。选择规则: 取正分最高的 debt area; 同分按 `tooling-ci > ui-ux > testability > performance > dependency > docs > architecture`; `architecture` 只有自身 area debt `>=30` 时才能自动选择, 否则跳过到下一个 area。byArea 输出带水位标记 (`!` 过推荐线 / `~` 过 notice)。
...
23. 用户已明确授权推进 ("继续 / 直接开始落地" 类信号) 后, 阶段边界不插入变相确认 (方案汇报 + 停手等回应也算); 只在硬阻塞 (PRD 级歧义 / 不可逆外部副作用 / 范围根本不清) 停。需要用户拍板的离散决策一次性问完 (单个 AskUserQuestion 批次) 再开干, 不在执行链路反复回头 (friction 20260622-confirm-after-explicit-go, 高影响用户偏好, 另见 memory)。
