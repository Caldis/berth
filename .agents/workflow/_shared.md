...
11. 已验证、边界清楚的增量必须小步频繁提交并及时推送; 每次提交前只暂存自己相关文件, 用 `git diff --cached` 核对 staged 集合, 不提交无关工作区改动。共享工作区禁止 `git add -A`、`git add .` 和目录级批量 add; 显式列出本轮处理过的文件。提交成功后, 先运行本轮所需本地检查; 代码类提交优先运行 `pnpm harness:prepush`, 但如果该命令只因远端 CI baseline 未完成而阻塞, 必须拆开运行本地 lint/typecheck/test/harness 检查, 不把远端等待当成实现阶段同步门禁。确认可以推送后运行 `git push` 推送当前分支, 先用 `git rev-parse HEAD` 读取完整 SHA, 再由子代理或等价旁路检查异步跟踪该 SHA 的 GitHub Actions 状态; 旁路检查可使用 `pnpm harness:ci:wait --sha <full-sha>`, 但由子代理或等价旁路执行, 主 Agent 不同步等待 CI 完成, 继续做不依赖 CI 结果的当前实现。若旁路检查报告当前会话提交导致 CI failure, 主 Agent 先做归因: 失败严重影响当前任务主线或后续实现基础时, 立即转入 CI 修复; 失败与当前任务无关、仍在运行、或可在 verify 统一处理时, 记录状态并继续当前开发。CI 修复提交允许在红灯基线下推送, 但必须明确它是在修复红灯, 并用 `pnpm harness:ci:baseline -- --allow-failed-baseline` 作为显式例外。verify/archive/最终完成声明前, 主 Agent 必须消费成功结果, 包括远端 CI 成功和 Project 同步结果; 若远端拒绝、网络失败或 Actions 暂不可用, 记录原因后继续可本地验证的工作, 但收口前先处理远端状态。仅改进度状态类文档时, 例如 `INDEX.md` phase、`03-PLAN.md` 复选框或测试证据, 不单独提交, 等下一次代码、测试、设计产物、归档或其他阶段结果一起提交并推送; 若这类文档是当前阶段唯一剩余变更, 则在阶段结束、归档或停手前提交。若误提交了他人文件, 用 `git reset --soft HEAD~1` 拆回索引, 再 `git restore --staged <path>` 逐个踢出, 不用 destructive reset。
