...
- [~] **T2 单管线 + changeset 协议 + SQLite** (de-risk + 持久层落地; 增量写协议待续)
  ...
  - [ ] 余 (issue 2026-06-08-IMPROVEMENT-incremental-write-followups): `deriveAssetsForPath` 扩展能力文件 (skill/agent/command/output-mode glob + settings.json/.mcp.json/config.toml/hooks.json 单文件多资产, 按 sourceKey 整体替换); 真实 chokidar 端到端 e2e; renderer `assets:changed` dead 订阅清理; 行级 SQLite delta (`replaceBySourceKey` 替代全量 save); worker parse→changeset→main 单 writer (背压, 实测驱动); sidecar 依赖图; parse-error 保留旧行 `source_status=error`; 单调 checkpoint。
