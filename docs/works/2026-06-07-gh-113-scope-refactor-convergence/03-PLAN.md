...
- [~] **T2 单管线 + changeset 协议 + SQLite** (de-risk + 持久层落地; 增量写协议待续)
  ...
  - [ ] **cap-4 真实 e2e + 收尾** (issue followups ②③④): 真实 chokidar 端到端 e2e (改 CLAUDE.md/新增 SKILL.md → 轮询 snapshot 增量); renderer `assets:changed` dead 订阅清理; 行级 SQLite delta `replaceBySourceKey` (替代全量 save)。
