...
- [~] **T2 单管线 + changeset 协议 + SQLite** (de-risk + 持久层落地; 增量写协议待续)
  ...
  - [ ] **cap-1 deriveAssetsForPath 单文件多资产**: `.mcp.json`/`.claude/settings.json`/`settings.local.json`/`.codex/config.toml`/`.codex/hooks.json`, 复用 settings 组合 parser, scope 走 inferScope (project/user), 按 sourceKey 整体替换其全部派生。
  - [ ] **cap-2 deriveAssetsForPath glob 类单资产**: skill/agent/command/output-mode (`.claude/*` 目录) + codex skill (`.agents/skills`)/customAgent (`.codex/agents`), 按目录模式匹配, 单文件重 parse 出单 asset。
  - [ ] **cap-3 特例 + e2e**: enterprise (managed-*.json) / plugin (user scope + plugin-* 前缀) / sidecar (.berth/hooks-state.json) scope 推断; 真实 chokidar 端到端 e2e (issue #2); renderer `assets:changed` dead 订阅清理 (issue #3); 行级 SQLite delta `replaceBySourceKey` (issue #4)。
