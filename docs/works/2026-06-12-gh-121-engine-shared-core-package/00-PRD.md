...
来源:
- `docs/issues/2026-06-09-IMPROVEMENT-engine-shared-core-package.md` (快照于 2026-06-12, 0.0-new)
- GitHub Issue: https://github.com/Caldis/berth/issues/121
- 用户指令 (2026-06-11/12): 重构链按序执行 — "窗口加固 (GH-119, 已归档) → ① engine 成包 → ② 拆 runtime (asset-runtime-collaborators-split) → ③ indexer 主线剩余 (长驻 worker/调度背压)"; 本任务为链 ①, 完成后推送。链顺序依据: 迁包是后续 engine 重构的安全前置 (CLI 相对路径 + typecheck 盲区), 拆 runtime 的 ScanCoordinator 是链 ③ 落点。
