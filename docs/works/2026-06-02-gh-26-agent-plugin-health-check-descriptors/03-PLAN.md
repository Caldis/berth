...
- [x] 任务 5: 修正 health descriptor i18n key, 避免冒号被 i18next 解析为 namespace。
  ...
  - evidence: passed 2026-06-02, agent capability plugin unit 10 tests plus typecheck.
...
- 2026-06-02: code review 发现 descriptor i18n key 直接拼接冒号 id, i18next 默认会把冒号解析为 namespace 分隔符。已追加任务 5, phase 退回 implement。
  修复后目标单测与 typecheck 通过, phase 回到 verify。
