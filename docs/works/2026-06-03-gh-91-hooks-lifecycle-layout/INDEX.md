...
debt:
  ...
  final:
    ...
    areas:
      - ui-ux
    confidence: medium
    rationale: "Renderer shell 使用全局 page gutter、顶部偏移和 scrollbar gutter 变量, Hooks 生命周期左栏使用同一偏移计算 sticky top/max-height; connector 样式和滚动重算节流仍限于 Hooks renderer 组件; 主进程、IPC 与数据契约未变。"
  revisions:
    - phase: explore
      date: 2026-06-03
      from:
