...
- [x] T3 renderer 数据层 + 页面四态: use-agent-teams.ts hook (模块缓存 + mount 刷新); pages/teams.tsx 骨架 (PageChrome title/subtitle/guide + loading/empty/error/数据四态); feature-guidance.ts teamsGuide; i18n en/zh 键
...
- [x] T4 路由 + 导航 + redirect: App.tsx /teams 路由 + 旧 redirect 改指 /teams; nav-config work 区追加 teams 项 (Users 图标)
...
## 实现偏差记录
- T3/T5 合并实现: 页面四态与卡片明细是同一 JSX 单元, 人为分两轮提交会产生一次性中间态, 故一次落全并在同一测试文件覆盖 (四态 + 卡片明细 + 阈值 + 跳转 + nav)。
