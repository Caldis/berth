...
- [x] 任务 13: Cursor 真实 adapter v1
  - scope: `cursor` 不再只走 metadata-only declared adapter; 新增真实 adapter/parser, 读取稳定低风险本地/项目文件: `.cursor/rules/**/*.{md,mdc}`、`AGENTS.md`、`.cursor/skills/**/SKILL.md`、`.cursor/agents/**/*.md`、`.cursor/commands/**/*.md`、`.cursor/mcp.json`、`.cursor/hooks.json`、`.cursor/permissions.json`、`.cursor/sandbox.json`、`.cursor/plugins/**/{plugin.json,manifest.json,package.json}`。`subagents` 和 IDE state 仍只做 source coverage metadata, 不读取正文。
  - tests: `pnpm vitest run tests/unit/agent-adapter-registry.test.ts tests/unit/planned-agent-adapter-definitions.test.ts tests/unit/agent-capability-plugins.test.ts`; `pnpm vitest run tests/unit/engine-scanner.test.ts tests/unit/scope-dedupe.test.ts tests/unit/search.test.ts tests/unit/asset-dedupe.test.ts`; `pnpm --filter @berth/scan-engine test`; `pnpm --filter @berth/scan-engine typecheck`; `pnpm --filter @berth/scan-engine build`; `pnpm typecheck:node`; `pnpm typecheck:test`; `pnpm typecheck:web`; `pnpm lint`; `pnpm harness:check`。
  - verify: passed 2026-06-13。本机 `where.exe cursor` 和 `where.exe agent` 均未找到命令。用已构建 `packages/berth-scan-engine/dist/cli.cjs scan --home-dir $env:USERPROFILE --project D:\Code\berth --json` 汇总 Cursor 相关扫描: `cursorAssetCount=1` (`agents-md=1`), `cursorRootCount=1`, scanned source 为 `cursor.project.agents-md`, Cursor 相关 errors=0。因为当前项目存在 `AGENTS.md`, source coverage 会如实显示这个 Cursor 可读共享指导文件; 未发现用户级 `.cursor` 配置。

- [x] 任务 14: OpenClaw 真实 adapter v1
  - scope: `openclaw` 不再只走 metadata-only declared adapter; 新增真实 adapter/parser, 读取稳定低风险本地文件: `openclaw.json` 中的 MCP servers、workspace 指令文件、`SKILL.md`、extensions/plugins manifest、`sessions.json` 元数据摘要, 以及 secrets/auth/credentials 的 credential presence。session jsonl、trajectory、logs 仍只做 source coverage metadata, 不读取正文。配置解析先支持 JSON/JSONC 子集; 完整 JSON5 解析留作后续独立增量。
  - tests: `pnpm vitest run tests/unit/agent-adapter-registry.test.ts tests/unit/planned-agent-adapter-definitions.test.ts tests/unit/agent-capability-plugins.test.ts`; `pnpm vitest run tests/unit/engine-scanner.test.ts tests/unit/scope-dedupe.test.ts tests/unit/search.test.ts tests/unit/asset-dedupe.test.ts`; `pnpm --filter @berth/scan-engine test`; `pnpm --filter @berth/scan-engine typecheck`; `pnpm --filter @berth/scan-engine build`; `pnpm typecheck:node`; `pnpm typecheck:test`; `pnpm typecheck:web`; `pnpm lint`; `pnpm harness:check`。
  - verify: passed 2026-06-13。本机 `where.exe openclaw` 未找到命令。用已构建 `packages/berth-scan-engine/dist/cli.cjs scan --home-dir $env:USERPROFILE --project D:\Code\berth --json` 汇总 OpenClaw 相关扫描: `openClawAssetCount=28` (`skill=28`), `openClawRootCount=1`, scanned source 为 `openclaw.user.shared-skills`, OpenClaw 相关 errors=0。当前结果来自 `~/.agents/skills` 共享 skills, 不代表 OpenClaw CLI 已安装。

- [x] 任务 15: Hermes Agent 真实 adapter v1 与 detected 语义修正
  - scope: `hermes-agent` 不再只走 metadata-only declared adapter; 新增 `<hermes-home>` path resolver, 支持 `HERMES_HOME` 和 Windows `%LOCALAPPDATA%\hermes` 默认路径。新增真实 adapter/parser, 读取稳定低风险文件: `config.yaml` 中的 MCP servers/hooks、`SOUL.md`、Hermes 项目 context、`SKILL.md`、plugin manifest、hook manifest、`sessions.json` 元数据摘要, 以及 `.env`/`auth.json` credential presence。`state.db`、logs、cache、browser recordings、checkpoints 仍只做 source coverage metadata, 不读取正文。`CLAUDE.md`、`.cursor/rules` 作为 Hermes 兼容 source coverage 展示, 不重复产出资产。
  - detected 修正: 第三方 adapter 的 `detect().installed` 只由 agent 自有 home/config 或专属项目配置判定; 共享 `AGENTS.md`、共享 `.agents/skills`、`CLAUDE.md` 仍显示 source coverage, 但不再让 Cursor/OpenClaw/Hermes 等误显示为已检测安装。
...
- [x] 任务 16: agent home/config 环境变量与真实扫描闭环
  ...
  - package smoke: passed 2026-06-13。用 `npx` 非全局安装方式执行版本命令: `@google/gemini-cli@latest --version` = `0.46.0`, `@github/copilot@latest version` = `GitHub Copilot CLI 1.0.61`, `opencode-ai@latest --version` = `1.17.4`, `openclaw@latest --version` = `OpenClaw 2026.6.6 (8c802aa)`。`python -m pip index versions hermes-agent` 显示 PyPI latest `0.15.2`; 未修改全局 Python 环境。
