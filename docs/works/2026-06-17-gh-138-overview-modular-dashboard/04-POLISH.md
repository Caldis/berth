...
- [ ] U4 用量趋势 5d→可配置且 ≥30d 起。
- [ ] U1 活动热力图未占满右侧横向空间 → cell 随宽 flex 填充。
- [ ] U2 洞察/常用 样式空洞 → 增密度/视觉充实。
- [ ] U6 M/L 尺寸无肉眼区别 → 跨度/高度可辨。
- [ ] U7 瀑布流填满空间 + 垂直组件动态高度 (3M+1Wide 不留空格) — 架构级。
- [ ] U5 隐藏 widget → 底部 sticky 毛玻璃浮动画廊, 预览渲染(非标题), 可直接拖出 (iOS 小组件库) — 架构级。
