...
- 以 `@radix-ui/react-popover` 为底层 primitive。
- 公开能力:
  - `trigger`: trigger JSX。
  - `children`: popover 内容。
  - `side` / `align` / `sideOffset` / `collisionPadding`: 透传给 Radix Content。
  - `triggerTestId` / `contentTestId`: 保持现有测试锚点。
  - `contentClassName` / `triggerClassName`: 让调用点保留已有尺寸和视觉 token。
  - hover/focus/click 打开策略: 共享组件内统一处理 open state、延迟关闭和 portal content 事件。
...
- `package.json` / `pnpm-lock.yaml`: 新增 `@radix-ui/react-popover` devDependency, 与现有 Radix 依赖分组一致。
- `src/renderer/src/components/shared/floating-popover.tsx`: 公共浮层组件, 只负责 Radix Popover、Portal、定位、打开/关闭与基础样式; 不包含业务文案。
...
| 项目 | 方案 | 验收方式 |
|---|---|---|
| 布局层级 / 信息密度 | 浮层内容通过 Radix Portal 渲染到 body, 不再受 header/sidebar overflow 限制; 内容结构和密度基本不变。 | renderer 结构断言 + Electron 截图 |
| 组件选择 / 设计系统一致性 | 使用 Radix Popover; 样式继续使用 `bg-popover`、`border-border`、`shadow-lg` 和现有圆角。 | typecheck + 视觉检查 |
| 交互反馈 / 状态切换 | hover/focus 打开, 离开延迟关闭; click 仍可打开 header 富内容。 | renderer hover 测试 |
...
|---|---|
| 新增 `FloatingPopover` + Radix Popover Portal | 1, 2, 3, 4 |
