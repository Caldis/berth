...
- [ ] 任务 1: 加 `ignore` 依赖 + 跨进程 `respectGitignore` 字段打通
...
- [ ] 任务 2: `engine/scan-ignore.ts` 模块 (loadProjectIgnore + buildScanIgnore)
...
- [ ] 任务 5: batchPauseMs 背压 spy 测 (issue 第三项)
  ...
  - tests: `tests/scanner-backpressure.test.ts` (新): sleep spy, 断言 batchPauseMs>0 时 adapter 间调用 sleep, =0 时不调 (AC7)。
