...
- [ ] 任务 1: 在 health-check i18n helper 中增加 scope / confidence / asset type 标签本地化, 并补充 en/zh 文案。
  - tests: `pnpm vitest run tests/renderer/overview-health-checks.test.tsx tests/renderer/hooks-lifecycle-view.test.tsx`
  - verify: 中文 tag 不再显示 raw 英文枚举; 未知值 fallback 原值。
- [ ] 任务 2: 接入 Overview 与 Hooks 生命周期 hover 详情。
  - tests: `pnpm vitest run tests/renderer/overview-health-checks.test.tsx tests/renderer/hooks-lifecycle-view.test.tsx`
