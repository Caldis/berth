...
export const PLANNED_AGENT_ADAPTER_DEFINITIONS: AgentAdapterDefinition[] = [
  ...
  },
  {
    ...
    permissions: [readPermission('<copilot-home>', '<project>/.github', '<project>/AGENTS.md')],
    references: refs('Config directory', REF.copilotConfig, 'Commands', REF.copilotCommands),
    sources: [
      source('copilot.user.home', 'user', 'directory', ['instruction', 'capability', 'state', 'observability', 'integration'], '<copilot-home>', REF.copilotConfig),
      source('copilot.user.settings', 'user', 'file', ['capability'], '<copilot-home>/settings.json', REF.copilotConfig),
      source('copilot.user.config', 'user', 'file', ['integration'], '<copilot-home>/config.json', REF.copilotConfig, {
        sensitivity: 'credential-presence-only',
        maxBytes: 0
      }),
      source('copilot.user.permissions', 'user', 'file', ['capability'], '<copilot-home>/permissions-config.json', REF.copilotConfig, {
        sensitivity: 'sensitive-metadata-only'
      }),
      source('copilot.user.copilot-instructions', 'user', 'file', ['instruction'], '<copilot-home>/copilot-instructions.md', REF.copilotInstructions),
      source('copilot.user.mcp-config', 'user', 'file', ['capability'], '<copilot-home>/mcp-config.json', REF.copilotConfig),
      source('copilot.user.lsp-config', 'user', 'file', ['capability'], '<copilot-home>/lsp-config.json', REF.copilotCommands),
      source('copilot.user.instructions', 'user', 'directory', ['instruction'], '<copilot-home>/instructions', REF.copilotConfig),
      source('copilot.user.agents', 'user', 'directory', ['instruction'], '<copilot-home>/agents', REF.copilotAgents),
      source('copilot.user.skills', 'user', 'directory', ['instruction'], '<copilot-home>/skills', REF.copilotConfig),
      source('copilot.user.shared-skills', 'user', 'directory', ['instruction'], '~/.agents/skills', REF.copilotSkills),
      source('copilot.user.hooks', 'user', 'directory', ['capability'], '<copilot-home>/hooks', REF.copilotHooks),
      source('copilot.user.plugins', 'user', 'directory', ['capability'], '<copilot-home>/installed-plugins', REF.copilotPlugins),
      source('copilot.user.plugin-data', 'user', 'directory', ['state'], '<copilot-home>/plugin-data', REF.copilotPlugins, {
        sensitivity: 'sensitive-metadata-only'
      }),
      source('copilot.user.logs', 'user', 'directory', ['observability'], '<copilot-home>/logs', REF.copilotConfig, {
        sensitivity: 'debug-summary-only',
        defaultHidden: true
      }),
      source('copilot.user.command-history', 'user', 'directory', ['state'], '<copilot-home>/command-history-state', REF.copilotConfig, {
        sensitivity: 'sensitive-metadata-only'
      }),
      source('copilot.user.sessions', 'session', 'directory', ['state'], '<copilot-home>/session-state', REF.copilotConfig, {
        sensitivity: 'sensitive-metadata-only',
        maxRows: 200
      }),
      source('copilot.user.session-store', 'session', 'file', ['state'], '<copilot-home>/session-store.db', REF.copilotConfig, {
        sensitivity: 'sensitive-metadata-only',
        maxBytes: 10_000_000
      }),
  ...
  },
  {
    ...
    sources: [
      ...
      source('cursor.user.config', 'user', 'file', ['capability'], '<cursor-config>/cli-config.json', REF.cursorCliConfig),
      source('cursor.user.mcp', 'user', 'file', ['capability'], '<cursor-config>/mcp.json', REF.cursorMcp),
      source('cursor.user.hooks', 'user', 'file', ['capability'], '<cursor-config>/hooks.json', REF.cursorHooks),
      source('cursor.user.permissions', 'user', 'file', ['capability'], '<cursor-config>/permissions.json', REF.cursorPermissions),
      source('cursor.user.sandbox', 'user', 'file', ['capability'], '<cursor-config>/sandbox.json', REF.cursorPermissions),
      source('cursor.user.skills', 'user', 'directory', ['instruction'], '<cursor-config>/skills', REF.cursorSkills),
      source('cursor.user.agents', 'user', 'directory', ['instruction'], '<cursor-config>/agents', REF.cursorAgents),
      source('cursor.user.plugins', 'user', 'directory', ['capability'], '<cursor-config>/plugins', REF.cursorPlugins),
      source('cursor.user.subagents', 'user', 'directory', ['state'], '<cursor-config>/subagents', REF.cursorAgents, {
        sensitivity: 'sensitive-metadata-only',
        maxRows: 200
      }),
  ...
  },
  {
    ...
    versionProbe: { command: 'opencode', args: ['--version'], packageName: 'opencode-ai', source: 'cli' },
    permissions: [readPermission('<opencode-config>', '<opencode-data>', '<project>/.opencode', '<project>/opencode.json', '<project>/opencode.jsonc')],
    references: refs('Config', REF.opencodeConfig, 'Rules', REF.opencodeRules),
    sources: [
      source('opencode.user.config-jsonc', 'user', 'file', ['capability'], '<opencode-config>/opencode.jsonc', REF.opencodeConfig),
      source('opencode.user.config', 'user', 'file', ['capability'], '<opencode-config>/opencode.json', REF.opencodeConfig),
      source('opencode.user.assets', 'user', 'directory', ['instruction', 'capability'], '<opencode-config>', REF.opencodeConfig),
      source('opencode.user.agents', 'user', 'directory', ['instruction'], '<opencode-config>/agents', REF.opencodeConfig),
      source('opencode.user.commands', 'user', 'directory', ['instruction'], '<opencode-config>/commands', REF.opencodeConfig),
      source('opencode.user.skills', 'user', 'directory', ['instruction'], '<opencode-config>/skills', REF.opencodeConfig),
      source('opencode.user.plugins', 'user', 'directory', ['capability'], '<opencode-config>/plugins', REF.opencodePlugins),
      ...
      source('opencode.user.auth', 'user', 'file', ['integration'], '<opencode-data>/auth.json', REF.opencodeConfig, {
        sensitivity: 'credential-presence-only',
        maxBytes: 0
      }),
      source('opencode.user.sessions-db', 'session', 'file', ['state'], '<opencode-data>/opencode.db', REF.opencodeRepo, {
        stability: 'primary-source',
        sensitivity: 'sensitive-metadata-only',
        maxBytes: 10_000_000
      }),
      source('opencode.user.logs', 'user', 'directory', ['observability'], '<opencode-data>/log', REF.opencodeConfig, {
        sensitivity: 'debug-summary-only',
        defaultHidden: true
      })
    ],
  ...
  },
  {
    ...
    versionProbe: { command: 'openclaw', args: ['--version'], packageName: 'openclaw', source: 'cli' },
    permissions: [readPermission('<openclaw-state>', '~/.agents/skills')],
    references: refs('CLI', REF.openclawCli, 'Configuration', REF.openclawConfig),
    sources: [
      source('openclaw.user.config', 'user', 'file', ['capability'], '<openclaw-config>', REF.openclawConfig),
      source('openclaw.user.workspace', 'user', 'directory', ['instruction', 'capability'], '<openclaw-state>/workspace', REF.openclawConfig),
      source('openclaw.user.skills', 'user', 'directory', ['instruction'], '<openclaw-state>/skills', REF.openclawSkills),
      source('openclaw.user.shared-skills', 'user', 'directory', ['instruction'], '~/.agents/skills', REF.openclawSkills),
      source('openclaw.user.extensions', 'user', 'directory', ['capability'], '<openclaw-state>/extensions', REF.openclawPlugins),
      source('openclaw.workspace.extensions', 'user', 'directory', ['capability'], '<openclaw-state>/workspace/.openclaw/extensions', REF.openclawPlugins),
      source('openclaw.user.plugins', 'user', 'directory', ['capability'], '<openclaw-state>/plugins', REF.openclawRepo, {
        stability: 'heuristic'
      }),
      source('openclaw.user.credentials', 'user', 'directory', ['integration'], '<openclaw-state>/credentials', REF.openclawSecurity, {
        sensitivity: 'credential-presence-only',
        maxRows: 0
      }),
      source('openclaw.user.secrets', 'user', 'file', ['integration'], '<openclaw-state>/secrets.json', REF.openclawSecurity, {
        sensitivity: 'credential-presence-only',
        maxBytes: 0
      }),
      source('openclaw.user.auth-profiles', 'user', 'file', ['integration'], '<openclaw-state>/agents/default/agent/auth-profiles.json', REF.openclawSecurity, {
        stability: 'primary-source',
        sensitivity: 'credential-presence-only',
        maxBytes: 0
      }),
      source('openclaw.user.sessions-index', 'session', 'file', ['state'], '<openclaw-state>/agents/default/sessions/sessions.json', REF.openclawSessions, {
        sensitivity: 'sensitive-metadata-only',
        maxBytes: 10_000_000
      }),
      source('openclaw.user.sessions', 'session', 'directory', ['state'], '<openclaw-state>/agents/default/sessions', REF.openclawSessions, {
        sensitivity: 'sensitive-metadata-only',
        maxRows: 200
      }),
      ...
      source('openclaw.user.legacy-logs', 'user', 'directory', ['observability'], '<openclaw-state>/logs', REF.openclawCli, {
        stability: 'heuristic',
        sensitivity: 'debug-summary-only',
        defaultHidden: true
      })
  ...
  },
  {
    ...
    versionProbe: { command: 'hermes', args: ['version'], packageName: 'hermes-agent', source: 'cli' },
