...
export const featuresKo: Article[] = [
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Berth는 읽기 전용입니다. 이 파일들을 표시하기 위해 읽을 뿐, 결코 쓰지 않습니다. API 키 같은 자격 증명은 상태 확인 용도로만 감지되며 결코 표시되지 않습니다.' },
    ],
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: '지난 세션을 프로젝트별 또는 날짜별로 묶어 둘러봅니다. 각 세션은 불러온 skills, 연결한 MCP 서버, 발동한 hooks, 그리고 만들어 낸 산출물(계획, 할 일 목록, 파일 이력)을 보여 주며, 세션 전체를 이벤트 단위로 리플레이할 수도 있습니다. 사용자 메시지, 어시스턴트 응답, 사고 과정, 도구 호출이 Canvas 타임라인 위에서 순서대로 재생되며 유형 필터, 검색, 내보내기를 지원합니다.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Berth는 전적으로 당신의 기기 안에서 동작합니다. 텔레메트리도, 클라우드 동기화도, 계정도 없습니다. Berth는 결코 파일을 수정하지 않으며, 자격 증명은 상태 확인 용도로만 감지될 뿐 결코 표시되지 않습니다.' },
