...
export const featuresZh: Article[] = [
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Berth 是只读的:为展示而读取这些文件,绝不写入。API Key 等凭证仅用于探测状态,永不显示。' },
    ],
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: '按项目或时间浏览历史会话。每次会话会显示它加载的 Skill、连接的 MCP 服务、触发的 Hook,以及产出的产物(计划、待办、文件历史)——还可以逐事件重放整次会话:用户消息、助手回复、思考与工具调用在 Canvas 时间轴上按序回放,支持类型筛选、搜索与导出。' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'h2', text: '隐私与只读' },
      { type: 'p', text: 'Berth 完全在你的机器上运行:无遥测、无云同步、无账号。Berth 绝不修改任何文件,凭证仅用于探测状态——永不显示。' },
