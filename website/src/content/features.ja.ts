...
export const featuresJa: Article[] = [
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Berth は読み取り専用です。これらのファイルを表示するために読み取るだけで、書き込むことは決してありません。API キーなどの認証情報は状態確認のためだけに検出され、表示されることはありません。' },
    ],
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'プロジェクトや日付ごとにまとめられた過去のセッションを閲覧できます。各セッションには、読み込んだ Skills、接続した MCP サーバー、発火した Hooks、生み出した成果物 (プラン、ToDo、ファイル履歴) が表示され、さらにセッション全体をイベント単位でリプレイできます。ユーザーメッセージ、アシスタントの返答、思考、ツール呼び出しが Canvas タイムライン上で順番に再生され、種類フィルター、検索、エクスポートにも対応します。' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Berth は完全にあなたのマシン上で動作します。テレメトリーなし、クラウド同期なし、アカウントなし。Berth はファイルを決して変更せず、認証情報は状態確認のためだけに検出され、表示されることはありません。' },
