...
export const featuresEn: Article[] = [
  {
    ...
    body: [
      ...
      { type: 'p', text: 'v0.1 is read-only: Berth reads these files to display them and never writes to them. Credentials such as API keys are detected for status only and never shown.' },
    ],
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Browse past sessions grouped by project or date. Each session shows the skills it loaded, the MCP servers it connected, the hooks that fired, and the artifacts it produced (plans, todos, file history) — plus a tool timeline of what ran, in order.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'p', text: 'Berth runs entirely on your machine: no telemetry, no cloud sync, no account. v0.1 never modifies a file, and credentials are detected for status only — never displayed.' },
